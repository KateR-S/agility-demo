...
* [`src/cljs/agility_demo/`](src/cljs/agility_demo/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/agility_demo/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `agility-demo` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/agility_demo/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns agility-demo.example
  (:require [agility-demo.config :as config])
